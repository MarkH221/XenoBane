...
namespace PS3FileSystem
{
    ...
    {
        public static IEnumerable<string> StaticKeys
        {
            get
            {
                return new[]
        ...
        {
            return (from line in StaticKeys let x = line.Split('=')[0] where x.ToLower() == name.ToLower() select line.Split('=')[1] into value select StringToByteArray(value)).FirstOrDefault();
        }
